...
```{r}
train <- read.csv("C:/Users/tomas/Desktop/datos_proyecto/db_airline/train.csv")
```

```{r}
test <- read.csv("C:/Users/tomas/Desktop/datos_proyecto/db_airline/test.csv")
```
...
# Verificar correlaciones y tomar acción si son mayores a 0.7
threshold <- 0.7

# Iterar por cada par de variables
for (i in 1:(ncol(cor_matrix)-1)) {
  for (j in (i+1):ncol(cor_matrix)) {
    correlation <- cor_matrix[i, j]
    
    # Verificar si la correlación es mayor a 0.7
    if (abs(correlation) > threshold) {
      cat("Correlación entre", colnames(cor_matrix)[i], "y", colnames(cor_matrix)[j], "es mayor a 0.7\n")
      
      # Aquí puedes agregar el código adicional que deseas ejecutar para las correlaciones mayores a 0.7
    }
  }
}
```
...
```{r}
roc_metrics <- roc(df_test$satisfaction, predicciones_rl3)
...
y_pred2b[pred_logistic2 > 0.9877032 	] = 1
...
```{r}
train_knn2 <- df_train
train_knn2 <- df_train %>% dplyr::select(-Gender,-satisfaction,-Customer.Type,-Type.of.Travel,-Class,-Flight.Distance) 

test_knn2 <- df_test
test_knn2  <- df_test %>% dplyr::select(-Gender,-satisfaction,-Customer.Type,-Type.of.Travel,-Class,-Flight.Distance) 
```

Se nota que el óptimo de KNN se encuentra a accuracy=5 , por lo que el intercepto del eje x es k=5:

```{r}
overall.accuracy = c()
for (i in 1:20){
  knn.pred=knn(train_knn2,test_knn2,df_train$satisfaction,k=i)
  values = confusionMatrix(table(knn.pred,df_test$satisfaction))
  overall = values$overall
  overall.accuracy = append(overall.accuracy , overall["Accuracy"])
}
acc2 = data.frame(k=1:20, accuracy = overall.accuracy)
ggplot(acc2) + aes(x = k, y = accuracy) +geom_line(size = 0.5, colour = "#112446") +  theme_minimal() + geom_vline(xintercept = 5, color = "red")
```
...
Se nota que el óptimo de KNN se encuentra a accuracy=3 , por lo que el intercepto del eje x es k=3:
...
spec2 = data.frame(k=1:20, specificity = overall.specificity)
ggplot(spec2) + aes(x = k, y = specificity) +geom_line(size = 0.5, colour = "#112446") +  theme_minimal() + geom_vline(xintercept = 3, color = "red")
```
...
tree_pred
confusionMatrix(tree_pred,df_test$satisfaction,positive="satisfied")
```
...
tree_pred2
confusionMatrix(tree_pred2,df_test$satisfaction,positive="satisfied")
```
...
tree_pred3
confusionMatrix(tree_pred3,df_test$satisfaction,positive="satisfied")
```
...
tree_pred4
confusionMatrix(tree_pred4,df_test$satisfaction,positive="satisfied")
```
...
tree_pred5
confusionMatrix(tree_pred5,df_test$satisfaction,positive="satisfied")
```
...
tree_pred6
confusionMatrix(tree_pred6,df_test$satisfaction,positive="satisfied")
```
...
```{r}
df_train_svm <- df_train_rl
df_test_svm <- df_test_rl
```
...
Creacion de base de datos para iteración Kmeans
...
```{r}
rand_df2 <- sample(1:101307,50000,replace = F)
...
Para comprobar secuencia con resultados desagregados
...
for(i in muestreo3$Contador_Observaciones) {
    print(k2b[["cluster"]][[i]])
    print(df$satisfaction[df$X == muestreo3$X[muestreo3$Contador_Observaciones==i]])
    if (df$satisfaction[df$X == muestreo3$X[muestreo3$Contador_Observaciones==i]] == "satisfied") {
  resultado <- resultado + 1
    }
    total = total + 1

    print(muestreo3$X[muestreo3$Contador_Observaciones==i])
}
razon_k2 <- resultado/total
razon_k2
```
